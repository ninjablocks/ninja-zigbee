...
var ZigbeeLight = require(__dirname+'/ZigbeeLight');
var ZigbeeSmartPlug = require(__dirname+'/ZigbeeSmartPlug');
...
ZigBeeClient.prototype.processData = function(msg, msgPtr) {
  ...
  this.log.info('ZigBeeClient.prototype.processData: '+msg[msgPtr + SRPC_CMD_ID_POS]);
  switch (msg[msgPtr + SRPC_CMD_ID_POS]) {
    ...
    case RPCS_NEW_ZLL_DEVICE:
      ...
      // SUPER HACK
      var device;
      if(profileId == 0x0104 && deviceId == 0x0009) //ZLL
      {
        device = new ZigbeeSmartPlug(this.log, profileId, deviceId, nwkAddr, endPoint);
      }
      else {

        device = new ZigbeeLight(this.log, profileId, deviceId, nwkAddr, endPoint);
      }
