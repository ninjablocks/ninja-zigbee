...
var self = this;
  setTimeout(function() {
    self.processData(new Buffer([1,33,161,194,1,4,1,9,0,0,0,1,173,0,0,0,0,96,80,131,0,0,0,96,0,0,0,0,0,0,0,96,2,0,0]), 0);
  }, 1000);
};
...
var reader = binary.parse(new Buffer(msg))
    .word8('command')
    ...
    console.log("LENGTH! - " + reader.buffer().length);

  this.log.debug('ZigBeeClient.prototype.processData - Parsed message header : ', reader.vars);
  var device = DEVICES[''+reader.vars.networkAddress+reader.vars.endPoint]; // + cluster!
  this.log.debug('Found device : ' + device);

  switch (reader.vars.command) {

    case RPCS_NEW_ZLL_DEVICE:
      self.processNewDevice(reader);
      break;
    ...
    case RPCS_GET_DEV_LEVEL_RSP:
      ...
      if(device) {
        device.ninja.emit('data',JSON.stringify({bri:reader.vars.level}));
      }
   ...
   case RPCS_TEMP_READING:
     ...
     reader.word32lu('value');
      ...
      if(device) {
        this.log.info('emitting value data for ZigbeeDevice', reader.vars);
        device.ninja.emit('data',(reader.vars.value/100).toString())
      }
      else
      {
        this.log.info('ZigBeeClient.prototype.processData: ZigbeeDevice not defined.');
      };
      break;

   case RPCS_ZONESTATE_CHANGE:

      reader.word32lu('zoneState');
      var zoneState = readers.vars.zoneState;
      var thisZigbeeDevice = device;

      this.log.info('Received zoneState for ZigbeeDevice: ' + zoneState );
  ...
  return msg.length; // XXX: Is this correct? Could multiple messages be contained here?
};
 ...
 * @fires ZigbeeClient#device instance of ZigbeeDevice on new device found.
 */
ZigBeeClient.prototype.processNewDevice = function(reader) {
  ...
  if(reader.vars.profileId == 0xc05e) //ZLL
  {
    switch (reader.vars.deviceId)
    {
  		case (0x0000):
  		{
  			//An On off Light
  ...
  else if(reader.vars.profileId == 0x0104) //ZLL
  {
    switch (reader.vars.deviceId)
    {
  		case (0x0000):
  ...
  for( i =0; i < numTypes; i++)
  {
    ...
    for( j =0; j < numClusters[i]; j++)
    {
      ...
      var device = new ZigbeeDevice(this.log, reader.vars.profileId, reader.vars.deviceId, reader.vars.networkAddress, reader.vars.endPoint, Type[i], Name[i]);
      DEVICES[reader.vars.networkAddress.toString()+reader.vars.endPoint.toString()+Cluster[i][j]] = device;
