...
ZigBeeClient.prototype.readCommand = function(msg) {
  ...
  this.log.debug('Parsed message command:', P.inverted[reader.vars.command] || 'UNKNOWN', 'header : ', JSON.stringify(reader.vars));
  ...
  if (reader.vars.command == P.RPCS_NEW_ZLL_DEVICE) {
    ...
    reader.vars.ieeeHex = pad(reader.vars.ieee.toString(16), 16);
    ...
    if (!device && reader.vars.command == P.RPCS_ZONESTATE_CHANGE) {
      // ES: I've seen zonestate changes coming through without having seen a NEW_ZLL_DEVICE
      // come through before it... so we will just make a new device on the fly.

      this.log.info("Unknown IAS Zone has reported a zone state change. Creating new device on-the-fly.");
...
ZigBeeClient.prototype.permitJoin = function(pairingTime) {
  pairingTime = Math.max(pairingTime || 60, 255);
